...
import static streetprotectsystem.Modify.r1;

public class Trash extends JFrame {

    Container contentPane; // 컨테이너 클래스의 객체 선언
    DefaultListModel model;  // 자바에서 제공해주는 기본 모델
    JList list2;

    public Trash() {

        setTitle("양천구 쓰레기통 위치");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); //종료버튼 활성화
        setContentPane(new MyTrashPanel());
            ...
            try {
                ...
                try {
                    ...
                    for (int i = 0; i < data.size(); i += 2) {
                        ...
                        String sulChi = data_sulChi.get("설치위치").toString();
                        String Ddoro = data_Ddoro.get("도로(가로)명").toString();
                        r1.add(Ddoro + sulChi);
                        bw.write("\n" + r1.get(i / 2));
                    }
        ...
        jo.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                model = new DefaultListModel(); //model의 객체선언
                list2 = new JList(model);
                JScrollPane scroll = new JScrollPane(list2);
                for (Object s1 : Modify.r1) {
                    model.addElement(s1);
                }
                scroll.setBounds(160, 180, 500, 250); // 스크롤 (x ,y, 폭, 높이)
